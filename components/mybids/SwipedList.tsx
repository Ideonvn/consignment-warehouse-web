...
import type { SwipedLot } from "@/lib/hooks/useSwipedLots";

export function SwipedList({ direction }: { direction: SwipeDirection }) {
  const queryClient = useQueryClient();
  const { showToast } = useToast();
  const { data, isPending, error, refetch } = useSwipedLots(direction);
  const [bidTarget, setBidTarget] = useState<SwipedLot | null>(null);
  const now = useNow();
  ...
  return (
    <>
          ...
          return (
            ...
            >
              ...
              <div className="flex gap-2 border-t border-border p-3 pt-2">
                ...
                {open ? (
                  <Button className="flex-1" onClick={() => setBidTarget({ lot, auction })}>
                    Bid
                  </Button>
                ) : null}
              </div>
      ...
      <BidSheet
        lot={bidTarget?.lot ?? null}
        currency={bidTarget?.auction.currency_code ?? "ZAR"}
        open={bidTarget !== null}
